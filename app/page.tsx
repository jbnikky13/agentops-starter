...
export default function Home() {
  ...
  return <main>
    ...
    {analysis&&<section className="results"><div className="resulthead"><div><div className="eyebrow">CAMPAIGN BLUEPRINT</div><h2>{analysis.brand}</h2><p>{analysis.summary}</p></div><span className="status">{analysis.aiMode}</span></div>
      ...
      <div className="approval"><div><small>LAUNCH SAFETY</small><h3>Human approval required before ad spend</h3><p>AdPilot prepares recommendations and drafts. Platform connections and budget changes stay behind an explicit approval step.</p></div><button onClick={()=>alert('Campaign draft saved locally. Connect an ad platform in the next phase to publish.')}>SAVE CAMPAIGN DRAFT</button></div>
    </section>}
    <footer>ADPILOT AI · CAMPAIGN INTELLIGENCE · V1 MVP</footer>
  </main>
}
